...
public class MinPQ<T> {
    /**
     * This is taken from Lab10, credit to CS61B :)
     */
    private Node[] contents;
    private int size;

    public MinPQ() {
        contents = new MinPQ.Node[16];
    ...
    private void sink(int index) {
        ...
        while (leftIndex(index) <= size) {
            ...
            if (lIndex < size && min(lIndex, lIndex + 1) == lIndex + 1) {
                lIndex += 1;
            }
            if (min(index, lIndex) == index) {
                break;
            }
            swap(index, lIndex);
            index = lIndex;
    ...
    public void changePriority(T item, double priority) {
        int i = 1;
        while (i < size && !getNode(i).item().equals(item)) {
            i++;
        }
    ...
    private void resize(int capacity) {
        Node[] temp = new MinPQ.Node[capacity];
        temp[0] = null;
    ...
    public void testSwim() {
        ...
        pq.size = 7;
        for (int i = 1; i <= 7; i += 1) {
            pq.contents[i] = new MinPQ<String>.Node("x" + i, i);
        }
