...
public class MapServer {
    ...
    public static List<Map<String, Object>> getLocations(String locationName) {
        LinkedList<Map<String, Object>> db = new LinkedList<>();
        LinkedList<Node> list = graph.getLocation(locationName);
        for (int i = 0; i < list.size(); i++) {
            Node node = list.get(i);
            HashMap<String, Object> param = new HashMap<>();
            param.put("id", node.id);
            param.put("lat", node.lat);
            param.put("lon", node.lon);
            param.put("name", node.name);
            db.add(param);
        }
        return db;
    }
